...
| **多轮滚动扩展上下文** | 用 AX 精准滚动聊天区 `AXScrollBar`（不动侧边栏、不抢焦点），触发微信懒加载旧消息，**合并去重后读回到原位**——典型一次能读到 30-60 条历史 |
| **说话人 + 时间戳感知** | 输出 `我：…` / `对方：…` / `──【14:55】──` 的清洁格式，Claude 能判断回复对象和消息时间跨度 |
| **OCR 兜底 + 气泡边界识别** | Accessibility 读不到时改走 Apple Vision 截图识别；**按气泡间距自动分行**，每条消息独立成行，不会把 N 条消息糊成一长串 |
| **10 种风格预设** | 幽默 / 严肃 / 认真 / 正式 / 嬉皮笑脸 / 温柔 / 专业 / 高冷 / 暧昧 / 怼回去，一键切换 |
| **多风格并行生成** | 一次并发 3 种风格让你挑（通常 5-6 秒全部出来，比串行快 ~3 倍） |
| **模仿用户说话风格** | 把历史里「我：…」作为 few-shot，生成贴近你本人的语气 / 用词 / 长度 |
| **累计长度自动对齐** | 自动统计"上一次我发言之后，对方累计发了多少字"，把这个数作为 Claude 回复的目标长度（`±30%`）。对方连珠炮 5 短句时你不会被逼成只回"嗯" |
| **多句连发 (1-5)** | UI 里选句数 N，生成 N 条换行分隔的短消息；点「📤 发送」**按 0.4s 间隔依次粘贴+回车**，3 条消息约 2 秒发完 |
| **流式输出** | Claude 边想边显示，长回复不再傻等 |
| **自动发送** | 勾选后生成完直接把消息粘贴到微信输入框并回车——按一次热键消息就飞出去 |
...
- **微信的发送快捷键保持默认**：微信 → 设置 → 通用 里 **「用 ⌘+Enter 发送消息」保持未勾选**（默认就是如此）。本工具固定用 Enter 触发发送，若你改过这个设置，消息会粘贴进输入框但无法发送出去。
...
1. 在微信里打开某个联系人的聊天界面
2. 按 `⌘ ⇧ A`（或点面板里的「📥 读取微信」）
   - 聊天区会用 AX 精准上滚 2 次（只动聊天区，不碰侧边栏，读完自动还原滚动位置）
   - 对话内容填入「对话内容」文本框（格式如 `我：xxx` / `对方：xxx` / `──【14:55】──`）
3. 选一个风格（幽默 / 温柔 / 认真 / …），可选填「额外要求」
4. 在「句数」里选 1-5 —— 1 = 单条回复；2-5 = 生成 N 条独立短消息，依次 `0.4s` 间隔发送
5. 点「✨ 生成回复」或「🎲 对比 3 种风格」
6. 结果显示在下方「建议回复」/「🎲 3 候选对比」Tab 里，**自动复制到剪贴板**
7. 要发就按「📤 发送 (回车)」；多句时会按顺序粘贴+回车，每条间隔 0.4s

### 高级选项

- **句数**（默认 1）：生成 N 条独立短消息，每条一行，发送时依次独立发出
- **换风格自动重生成**（默认开）：有回复后点其它风格按钮 → 立刻用新风格重跑
- **自动发送**（默认关）：勾上后生成完直接粘贴 + 回车到微信，按一次热键消息就飞出去
- **模仿我的说话风格**（默认开）：把历史里「我：…」作为 few-shot，让生成贴近你本人
...
### 5. 多轮滚动扩展上下文（AX 精准控制 scrollbar）

早期用 PageUp 键盘事件触发滚动，但全局按键会被错误送到侧边栏等别的滚动区。现在改为直接写聊天区 `AXScrollArea` 的 `AXVerticalScrollBar.AXValue`：

```
pass 0: 读当前视图 → 记下 scrollbar 原值 (比如 1.0)
pass 1: AXSetValue(0.7) → 等 0.7s 懒加载 → 读
pass 2: AXSetValue(0.4) → 等 0.7s 懒加载 → 读
合并去重 + 按首次出现顺序                    → 比可见量多 30-60 条
最后恢复 scrollbar 到原值，用户视角零打扰
```

不触碰键盘、不抢前台、不影响侧边栏，精确恢复位置。

### 6. 累计长度对齐（回复字数自适应）

`styles.py:calculate_length_target()` 扫描聊天文本：

- 找到最近的 `我：` 行
- 统计它之后所有 `对方：` 消息的**累计字数**
- 把这个数作为"本轮目标长度"传给 Claude（允许 `±30%`）

对方连珠炮发 5 条短消息（合计 40 字）时，Claude 会输出一条约 30-50 字的回复，而不是只回"嗯"。

### 7. 多句连发（1-5 句）

UI 有个 `句数` Spinbox。N > 1 时 prompt 改走"多句模式"，要求 Claude 输出**恰好 N 行**换行分隔的短消息。`on_send` 按行切分，子线程里依次：粘贴 → 0.6s → Enter → 0.4s 间隔 → 下一条。3 条总共约 2 秒发完。

### 8. 键盘注入的"Enter 被当换行"陷阱

**现象**：调 `CGEventPost` 发 Enter（keycode 36, flags=0），WeChat 却当作 Shift+Enter 处理，消息全堆输入框不发。

**根因**：`CGEventCreateKeyboardEvent` 创建新事件时，**flags 字段不保证初始为 0**，可能继承前一个 ⌘V 事件的残留修饰符。

**修复**（`src/sender.py`）：
- 每个事件**永远显式调 `CGEventSetFlags`**（就算是 flags=0 也调），不依赖默认值
- ⌘V 改成完整序列：`Cmd-down → V-down(Cmd flag) → V-up(Cmd flag) → Cmd-up`，让 macOS 明确看到 Cmd 的按下和释放
- 用 `CGEventSourceCreate(kCGEventSourceStateHIDSystemState)` 作为事件源，更像真实硬件输入

### 9. macOS 26 (Tahoe) 的 TSM 线程陷阱 —— 本项目踩过两次
...
4. 风格 5×2 网格
5. 额外要求输入 + 句数 Spinbox
6. 生成按钮（✨生成 / 🔄换一条 / 🎲 对比 3 种 / 🔍查看 Prompt）
7. 选项 checkbox（自动重生成 / 自动发送 / 模仿我的说话风格）
...
`send_to_wechat` 会先激活 WeChat 再粘贴。如果激活失败或微信焦点在错误的聊天里，消息会发到错地方。**首次使用建议先用「📝 只粘贴不回车」**，确认内容+目标对话都对，再改回「自动发送」。

### 多条消息全堆在输入框没有发送

说明 Enter 没触发发送。检查两件事：
1. 微信 → 设置 → 通用 → 「用 ⌘+Enter 发送消息」是否**被勾选**？如果是，请**取消勾选**（本工具固定用 Enter 发送）
2. 终端日志里 `[sender] 激活后前台 app = ?` 显示的是不是微信？如果不是，有别的 app 抢了焦点
